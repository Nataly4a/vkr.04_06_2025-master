<template>
  ...
  <div class="pick-up-time">
    <h1 class="section-title">Во сколько заберете пассажиров?</h1>
    <input type="time" id="pick-up-time" v-model="pickUpTime" />
    <button @click="submitPickUpTime">Подтвердить</button>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    if (!tripData) {
      // Если данных нет, перенаправляем на страницу выбора даты
  ...
  methods: {
    submitPickUpTime() {
      if (!this.pickUpTime) {
        alert("Пожалуйста, выберите время отбытия!");
        return;
      }
      ...
      // Обновляем данные о поездке с выбранным временем
      const updatedTrip = {
        ...tripData,
      ...
      // Переход на страницу выбора даты прибытия
      this.$router.push({ name: "ArrivalDate" }).catch((error) => {
